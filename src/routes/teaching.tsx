  ...
  head: () => ({
    meta: [
      { title: "Teaching Workflow · TeacherOS" },
      {
        name: "description",
        content:
          "Weekly teaching execution system with attendance capture, lesson tracking, and reflection logging.",
      },
    ],
  }),
...
function TeachingPage() {
  ...
  const completedClasses = week.reduce(
    (sum, d) => sum + d.classes.filter((c) => c.status === "completed").length,
    0,
  );
  ...
  const attendanceDone = week.reduce(
    (sum, d) => sum + d.classes.filter((c) => c.attendanceCaptured).length,
    0,
  );
  const reflectionsDone = week.reduce(
    (sum, d) => sum + d.classes.filter((c) => c.reflectionDone).length,
    0,
  );
  ...
  return (
    <div className="px-4 lg:px-8 py-6 lg:py-8 max-w-[1600px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          label="DLL Linked"
          value={`${dllLinked}/${totalClasses}`}
          icon={BookOpen}
          color="info"
        />
        <StatCard
          label="Attendance"
          value={`${attendanceDone}/${totalClasses}`}
          icon={Users}
          color="warning"
        />
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {week.map((d, i) => (
          <button
            ...
            className={cn(
              "shrink-0 rounded-lg border px-4 py-3 text-left min-w-[140px] transition-all",
              selectedDay === i
          ...
          <div className="rounded-xl border border-border bg-card">
            <div className="p-4 border-b border-border flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">
                {day.classes.filter((c) => c.status === "completed").length}/{day.classes.length}{" "}
                done
              </span>
            </div>
            ...
            <div className="rounded-xl border border-primary/30 bg-gradient-to-br from-primary-soft to-card p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li className="flex gap-2">
                  ...
                  <span>
                    DLL for Math 6 is ready —{" "}
                    <button className="text-primary hover:underline">Review now</button>
                  </span>
                </li>
                <li className="flex gap-2">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-3">
              ...
              <ProgressItem
                ...
                current={attendanceDone}
                total={totalClasses}
                color="bg-info"
              />
              <ProgressItem
                label="Reflections Done"
                current={reflectionsDone}
                total={totalClasses}
                color="bg-warning"
              />
            </div>
          ...
          <div className="rounded-xl border border-primary/20 bg-primary-soft/40 p-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              AI gauges class mastery after each lesson. Currently tracking 3 subjects.
            </p>
            <div className="mt-3 space-y-2 text-xs">
              <div className="flex justify-between">
                <span>Math 6</span>
                <span className="text-success font-medium">82% mastery</span>
              </div>
              <div className="flex justify-between">
                <span>Science 6</span>
                <span className="text-warning-foreground font-medium">74% mastery</span>
              </div>
              <div className="flex justify-between">
                <span>English 6</span>
                <span className="text-primary font-medium">88% mastery</span>
              </div>
            </div>
          </div>
        </div>
