  ...
  head: () => ({
    meta: [
      { title: "School Programs · TeacherOS" },
      {
        name: "description",
        content:
          "Plan, execute, and report on Feeding Program, DRRM, Brigada Eskwela and other school programs.",
      },
    ],
  }),
      ...
      <div className="flex gap-3 overflow-x-auto pb-2">
        {mockPrograms.map((p) => (
          <button
            ...
            className={cn(
              "shrink-0 rounded-xl border p-4 text-left min-w-[220px] transition-all",
              selected.id === p.id
          ...
          <div className="rounded-xl border border-border bg-card p-4 flex items-center gap-4">
            {(["planning", "execution", "reporting", "completed"] as const).map((s, i) => (
              <div key={s} className="flex items-center gap-2 flex-1">
                <div
                  className={cn(
                    ...
                    program.status === s
                      ...
                      : ["planning", "execution", "reporting", "completed"].indexOf(
                            program.status,
                          ) > i
                ...
                >
                  {["planning", "execution", "reporting", "completed"].indexOf(program.status) >
                  i ? (
                ...
                <div className="space-y-2">
                  {program.tasks.map((task, i) => (
                    <div key={i} className="flex items-center">
                      <div className="w-48 shrink-0 text-xs font-medium truncate pr-3">
                        {task.name}
                      </div>
                      <div className="flex-1 flex gap-0.5">
                        {weekLabels.map((_, wi) => {
                          const isActive =
                            wi + 1 >= task.startWeek && wi + 1 < task.startWeek + task.duration;
                          return (
                            <div
                              key={wi}
                              className={cn(
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">₱{program.spent.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{program.tasks.length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Progress</span>
                <span className="font-medium">
                  {Math.round(
                    (program.tasks.filter((t) => t.status === "done").length /
          ...
          <div className="rounded-xl border border-primary/20 bg-primary-soft/40 p-5">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Generate a post-activity accomplishment report with AI from your execution logs.
            </p>
