...
import {
  BookOpen, ClipboardCheck, Calculator, FileText, FileSignature, AlertTriangle,
  Play, Clock, Sparkles, ArrowRight,
} from "lucide-react";
...
export const Route = createFileRoute("/agents")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Multi-agent system for lesson planning, grading, forms, reports, and student risk analysis." },
    ],
  }),
            ...
            return (
              <button key={a.id} onClick={()=>setSelected(a.id)}
                className={cn("w-full text-left rounded-xl border p-4 transition-all flex items-start gap-3",
                  active ? "border-primary bg-primary-soft shadow-elegant" : "border-border bg-card hover:border-input")}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
                    ...
                    <span>·</span><span>{Math.floor(Math.random()*40)+5} runs today</span>
                  </div>
                </div>
              </button>
            );
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="rounded-xl border border-border bg-card p-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs text-primary font-semibold uppercase tracking-wider">Active agent</div>
                <h2 className="text-xl font-display font-semibold mt-1">{agent.name}</h2>
                <p className="text-sm text-muted-foreground">{agent.desc}</p>
              </div>
              <Button onClick={run} disabled={running} className="gap-2">
                {running ? <><Sparkles className="size-4 animate-pulse"/>Running…</> : <><Play className="size-4"/>Run workflow</>}
              </Button>
            </div>
            ...
            <div className="mt-5 grid sm:grid-cols-2 gap-4">
              ...
              <div>
                <div className="text-xs font-semibold text-muted-foreground mb-1.5">Memory Context</div>
                <div className="rounded-md border border-border bg-muted/40 p-3 text-xs space-y-1.5">
                  <div className="flex justify-between"><span className="text-muted-foreground">Grade level</span><span className="font-medium">6</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Section</span><span className="font-medium">Sampaguita</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Subject</span><span className="font-medium">Mathematics</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Past DLLs available</span><span className="font-medium">12</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">Linked module</span><span className="font-medium text-primary">Lesson Planning</span></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="font-mono text-xs space-y-1.5 min-h-[160px]">
                {log.length === 0 && <p className="text-muted-foreground">Click "Run workflow" to see live execution.</p>}
                {log.map((l, i) => (
                  <div key={i} className={cn(l.startsWith("✓") ? "text-success" : "text-foreground/80")}>{l}</div>
                ))}
              </div>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <ul className="space-y-2 text-xs">
                {["6:12 AM · Daily briefing", "Yesterday 4:30 PM · Math 6 DLL", "Yesterday 2:15 PM · Worksheet pack", "Mon · Quarterly assessment"].map(r=>(
                  <li key={r} className="flex items-center justify-between p-2 rounded hover:bg-muted/40">
                    <span>{r}</span>
