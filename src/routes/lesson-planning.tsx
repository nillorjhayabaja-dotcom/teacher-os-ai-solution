...
import {
  Sparkles, Download, FileText, Wand2, ChevronRight, ChevronDown,
  Check, History, FileDown, Save,
} from "lucide-react";
...
export const Route = createFileRoute("/lesson-planning")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Generate DepEd DLLs aligned with MELC. Input → AI Generation → Review → Export → Archive." },
    ],
  }),
...
function LessonPlanningPage() {
  ...
  function toggleMelc(code: string) {
    setSelectedMELC((prev) => prev.includes(code) ? prev.filter(c => c !== code) : [...prev, code]);
  }

  function runGeneration() {
    setGenerating(true);
    setTimeout(() => { setGenerating(false); setGenerated(true); setStep(3); }, 1600);
  }
  ...
  return (
    <div className="px-4 lg:px-8 py-6 lg:py-8 max-w-[1600px] mx-auto space-y-6">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" className="gap-2"><History className="size-4"/>Version history</Button>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6 space-y-5">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              ...
              <Field label="Subject">
                <select value={subject} onChange={(e)=>setSubject(e.target.value as keyof typeof mockMELCs)} className={inputCls}>
                  {Object.keys(mockMELCs).map(s => <option key={s}>{s}</option>)}
                </select>
              </Field>
              <Field label="Quarter">
                <select value={quarter} onChange={(e)=>setQuarter(e.target.value)} className={inputCls}>
                  <option>Quarter 1</option><option>Quarter 2</option>
                  <option>Quarter 3</option><option>Quarter 4</option>
                </select>
              </Field>
            </div>

            <div>
              <div className="text-xs font-semibold text-muted-foreground mb-2">MELC Competencies</div>
              <div className="rounded-lg border border-border divide-y divide-border">
                {melcList.length === 0 && <p className="p-4 text-sm text-muted-foreground">No MELCs seeded for this combination. Try Mathematics · Quarter 1.</p>}
                {melcList.map(m => {
                  const on = selectedMELC.includes(m.code);
                  return (
                    <button key={m.code} onClick={()=>toggleMelc(m.code)}
                      className={cn("w-full flex items-start gap-3 p-3 text-left hover:bg-muted/40", on && "bg-primary-soft")}>
                      <div className={cn("mt-0.5 size-4 rounded border grid place-items-center shrink-0",
            ...
            <div className="flex justify-end">
              <Button onClick={()=>setStep(2)} disabled={selectedMELC.length===0} className="gap-2">
      ...
      {step === 2 && (
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">The Lesson Planning Agent will draft Objectives, Procedure, Assessment, Differentiation, and Reflection sections aligned to your selected MELCs.</p>

            <div className="rounded-lg bg-muted/40 p-4 text-sm">
              <div className="font-medium">Context</div>
              <div className="mt-1 text-muted-foreground">Grade {grade} · {subject} · {quarter} · {selectedMELC.length} MELC(s)</div>
            </div>

            <Button onClick={runGeneration} disabled={generating} className="gap-2 w-full sm:w-auto">
              {generating ? <><Sparkles className="size-4 animate-pulse"/>Generating…</> : <><Wand2 className="size-4"/>Run AI Generation</>}
            </Button>

            {generating && (
              <ul className="text-xs text-muted-foreground space-y-1.5 pt-2">
                <li className="flex items-center gap-2"><Check className="size-3 text-success"/> Parsing MELC codes…</li>
                <li className="flex items-center gap-2"><Check className="size-3 text-success"/> Matching curriculum guide…</li>
                <li className="flex items-center gap-2"><Sparkles className="size-3 text-primary animate-pulse"/> Drafting procedure…</li>
              </ul>
            )}
          </div>
          <AsideTips />
            ...
            <div className="rounded-xl border border-border bg-card">
              <div className="flex items-center justify-between p-4 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                  <Button size="sm" variant="outline" className="gap-1.5"><Wand2 className="size-3.5"/>Improve</Button>
                  <span className="text-[11px] font-medium px-2 py-1 rounded bg-success/10 text-success flex items-center gap-1">
                    <Check className="size-3"/>MELC-aligned
                  </span>
                </div>
              </div>
              <div className="p-6 space-y-5 text-sm">
                <DLLSection title="I. Objectives" body="At the end of the lesson, learners will be able to: (a) add and subtract simple fractions with and without regrouping; (b) solve routine word problems involving fractions; (c) appreciate the use of fractions in daily Filipino life." />
                <DLLSection title="II. Content" body={`Subject Matter: ${subject} – ${quarter}\nMELCs: ${selectedMELC.join(", ")}`} />
                <DLLSection title="III. Learning Resources" body="K-12 Curriculum Guide, DepEd LR Portal, Realia (sliced bibingka), printed worksheets." />
                <DLLSection title="IV. Procedure" body="A. Reviewing previous lesson (5 min)\nB. Establishing purpose (5 min) — Story of pansit serving\nC. Presenting examples (10 min)\nD. Guided practice (15 min)\nE. Independent practice (10 min)\nF. Generalization (5 min)" />
                <DLLSection title="V. Assessment" body="10-item formative quiz; rubric-based group output (4 pts)." />
                <DLLSection title="VI. Reflection" body="No. of learners who earned 80% — ___\nNo. of learners who require additional activities — ___" />
              </div>
            </div>
            <div className="flex justify-between">
              <Button variant="outline" onClick={()=>setStep(2)}>Back</Button>
              <Button onClick={()=>setStep(4)} className="gap-2">Continue to Export <ChevronRight className="size-4"/></Button>
            </div>
          </div>
          <AsideTips />
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6 space-y-4">
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button onClick={()=>setStep(5)} className="gap-2">Archive lesson <ChevronRight className="size-4"/></Button>
            </div>
          </div>
          <AsideTips />
        </div>
      ...
      {step === 5 && (
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">Saved to your DLL library. Reuse or clone for next year.</p>
            ...
            {[
              ...
              <div key={v.v} className="flex items-center gap-4 p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{v.date} · {v.note}</div>
                </div>
      ...
      {generated && step !== 5 && (
        <div className="fixed bottom-6 right-6 rounded-lg border border-success/30 bg-card p-3 shadow-elegant flex items-center gap-2 text-sm">
          <Check className="size-4 text-success"/>Lesson autosaved
        </div>
      )}
    </div>
  );
...
const inputCls = "w-full h-9 px-3 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring/30";
...
function DLLSection({ title, body }: { title: string; body: string }) {
  ...
  return (
    <div>
      ...
      {open && <pre className="mt-2 whitespace-pre-wrap text-sm text-muted-foreground font-sans leading-relaxed">{body}</pre>}
    </div>
  );
}
