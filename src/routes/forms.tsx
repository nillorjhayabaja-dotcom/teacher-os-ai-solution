  ...
  head: () => ({
    meta: [
      { title: "School Forms Pipeline · TeacherOS" },
      {
        name: "description",
        content:
          "SF1–SF10 with AI auto-fill, validation, and submission tracking. DepEd-compliant layouts.",
      },
    ],
  }),
  component: FormsPage,
    ...
    <div className="px-4 lg:px-8 py-6 lg:py-8 max-w-[1600px] mx-auto space-y-6">
      ...
      {step === 1 && (
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground mb-5">
            Choose how to pull learner data into the form.
          </p>
          <div className="grid sm:grid-cols-3 gap-3">
            <SourceCard
              icon={Upload}
              title="Excel Upload"
              desc="LIS-formatted .xlsx"
              onClick={() => setStep(2)}
              active
            />
            <SourceCard
              icon={Database}
              title="LIS Import"
              desc="Direct from Learner Info System"
              onClick={() => setStep(2)}
            />
            <SourceCard
              icon={Pencil}
              title="Manual Entry"
              desc="Type student data"
              onClick={() => setStep(2)}
            />
          </div>
        </div>
      ...
      {step === 2 && (
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            AI scans the uploaded data and flags inconsistencies.
          </p>
          ...
          <ul className="mt-5 divide-y divide-border border border-border rounded-lg">
            <ValidationRow
              tone="error"
              title="LRN mismatch: J. Santos"
              desc="LRN length is 11 digits (should be 12)."
            />
            <ValidationRow
              tone="warn"
              title="Missing father's name: A. Reyes"
              desc="SF1 requires both parent names."
            />
            <ValidationRow
              tone="warn"
              title="Attendance mismatch: M. Cruz"
              desc="Manual register shows 18, LIS shows 17 days present."
            />
            <ValidationRow
              tone="warn"
              title="Grade gap: K. Bautista"
              desc="Q1 grade missing in MAPEH."
            />
          </ul>

          <div className="mt-5 flex justify-end">
            <Button onClick={() => setStep(3)} className="gap-2">
              Proceed with warnings <ChevronRight className="size-4" />
            </Button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            The Forms Agent fills every required field automatically.
          </p>
          <div className="mt-5 rounded-lg bg-muted/40 p-4 text-sm">
            ...
            <div className="text-muted-foreground mt-1">
              Section: Sampaguita · 40 learners · Month: September 2026
            </div>
          </div>
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-7 h-8 text-xs rounded border border-input bg-background w-44"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-between">
            ...
            <Button onClick={() => setStep(5)} className="gap-2">
              Review errors <ChevronRight className="size-4" />
            </Button>
          </div>
        </div>
      )}
      ...
      {step === 5 && (
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Errors highlighted in red. Accept AI suggestions or fix manually.
          </p>
          <div className="mt-5 divide-y divide-border border border-border rounded-lg">
            <FixRow
              problem="LRN: 137020025488 (11 digits)"
              suggestion="137020025488? Suggest verifying with LIS or appending check digit."
            />
            <FixRow
              problem="Father's name (blank) — A. Reyes"
              suggestion="Auto-fill from registration record: 'Roberto Reyes'."
            />
            <FixRow
              problem="Sept attendance: 17 vs 18"
              suggestion="Use register count: 18 (manual takes precedence per DO 8 s.2015)."
            />
          </div>
...
function SourceCard({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "rounded-lg border p-5 text-left transition-all hover:border-primary hover:shadow-elegant",
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-2 flex items-start gap-2 text-xs text-muted-foreground">
        ...
        <span>
          <span className="font-semibold text-foreground">AI suggests:</span> {suggestion}
        </span>
      </div>
        ...
        <table className="w-full mt-3 text-[11px] border-collapse">
          <thead>
            <tr className="bg-muted/60">
              ...
              <th className="border border-foreground/60 p-1 text-left">
                LEARNER'S NAME (Last, First, M.I.)
              </th>
              ...
              {[1, 2, 3, 4, 5, 8, 9, 10, 11, 12].map((d) => (
                <th key={d} className="border border-foreground/60 p-1 w-6">
                  {d}
                </th>
              ))}
              ...
              <tr key={s.id}>
                ...
                {[1, 2, 3, 4, 5, 8, 9, 10, 11, 12].map((d) => (
                  <td
                    key={d}
                    className={cn(
                      "border border-foreground/60 p-1 text-center",
                ...
                <td className="border border-foreground/60 p-1 text-center font-semibold bg-primary/5">
                  {s.attendance > 95 ? 20 : 19}
                </td>
              </tr>
            ))}
          </tbody>
