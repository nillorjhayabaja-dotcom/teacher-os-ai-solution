  ...
  head: () => ({
    meta: [
      { title: "Reports & Compliance · TeacherOS" },
      {
        name: "description",
        content:
          "RPMS, monthly accomplishment, narrative reports with AI drafting and evidence attachment.",
      },
    ],
  }),
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {types.map((type) => (
                <button
                  ...
                  onClick={() => {
                    setSelectedTemplate(
                      mockReportTemplates.find((t) => t.type === type.id) || mockReportTemplates[0],
                    );
                  }}
                  className={cn(
                    "rounded-xl border p-5 text-left transition-all hover:border-primary",
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {r.type} · Last edited: {r.lastEdited}
                      </div>
                    </div>
                    <span
                      className={cn(
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6 space-y-5">
            ...
            <p className="text-sm text-muted-foreground">
              Provide key details for the AI to generate your {selectedTemplate.name}.
            </p>
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">Quarter 1, Sy 2026-2027</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Teacher</span>
                <span className="font-medium">Mariel Reyes</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">School</span>
                <span className="font-medium">Sampaloc ES</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Division</span>
                <span className="font-medium">Bulacan</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {step === 3 && generating && (
        <div className="rounded-xl border border-border bg-card p-8 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Drafting narrative, formatting sections, and checking DepEd compliance.
          </p>
          <div className="mt-6 max-w-md mx-auto">
            ...
            <ul className="mt-4 text-xs text-muted-foreground space-y-1.5 text-left">
              <li className="flex items-center gap-2">
                <Check className="size-3 text-success" /> Analyzing input data…
              </li>
              <li className="flex items-center gap-2">
                <Sparkles className="size-3 text-primary animate-pulse" /> Drafting Introduction…
              </li>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-xl border border-border bg-card">
              ...
              <div className="p-6">
                <div className="text-sm whitespace-pre-wrap font-sans leading-relaxed text-foreground/90">
                  {draft}
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Drag & drop files or click to browse. Supported: PDF, DOCX, JPG, PNG, XLSX.
              </p>
              ...
              <div
                ...
                onDrop={(e) => {
                  e.preventDefault();
                  setDragging(false);
                  setEvidence((prev) => [...prev, "New_File.pdf"]);
                }}
                className={cn(
                ...
                {evidence.map((file, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Uploaded just now · Linked to: {selectedTemplate.name}
                      </div>
                    </div>
