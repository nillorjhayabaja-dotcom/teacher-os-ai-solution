...
export const Route = createFileRoute("/grading")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "DepEd-weighted quarter grading with AI anomaly detection and analytics.",
      },
    ],
  }),
...
function GradingPage() {
  ...
  const [anomalies, setAnomalies] = useState<
    { studentId: string; field: string; issue: string; fix: string }[]
  >([]);
      ...
      const updated = entries.map((e) => {
        ...
        if (e.ww1 === 0)
          detected.push({
            studentId: e.studentId,
            field: "WW1",
            issue: "Missing Written Work 1 score",
            fix: "Flag for review",
          });
    ...
    <div className="px-4 lg:px-8 py-6 lg:py-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/40 border-b border-border">
                  <th className="sticky left-0 bg-muted/40 text-left p-3 text-xs font-semibold text-muted-foreground uppercase min-w-[200px]">
                    Student
                  </th>
                  <th className="p-3 text-xs font-semibold text-muted-foreground uppercase text-center min-w-[80px]">
                    WW1
                    <br />
                  ...
                  <th className="p-3 text-xs font-semibold text-muted-foreground uppercase text-center min-w-[80px]">
                    PT1
                    <br />
                    <span className="font-normal text-[10px]">/30</span>
                  </th>
                  <th className="p-3 text-xs font-semibold text-muted-foreground uppercase text-center min-w-[80px]">
                    PT2
                    <br />
                  ...
                  <th className="p-3 text-xs font-semibold text-muted-foreground uppercase text-center min-w-[80px]">
                    Exam
                    <br />
                    <span className="font-normal text-[10px]">/50</span>
                  </th>
                  <th className="p-3 text-xs font-semibold text-muted-foreground uppercase text-center min-w-[90px] bg-primary/5">
                    Final
                  </th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={cn("p-2 text-center", e.ww1 === 0 && "bg-destructive/10")}>
                        <EditableCell
                          value={e.ww1}
                          onChange={(v) => updateEntry(e.studentId, "ww1", v)}
                        />
                      </td>
                      <td className="p-2 text-center">
                        <EditableCell
                          value={e.ww2}
                          onChange={(v) => updateEntry(e.studentId, "ww2", v)}
                        />
                      </td>
                      <td className={cn("p-2 text-center", e.pt1 > 30 && "bg-destructive/10")}>
                        <EditableCell
                          value={e.pt1}
                          onChange={(v) => updateEntry(e.studentId, "pt1", v)}
                        />
                        {e.pt1 > 30 && (
                          <AlertCircle className="size-3 text-destructive mx-auto mt-0.5" />
                        )}
                      </td>
                      <td className="p-2 text-center">
                        <EditableCell
                          value={e.pt2}
                          onChange={(v) => updateEntry(e.studentId, "pt2", v)}
                        />
                      </td>
                      <td className="p-2 text-center">
                        <EditableCell
                          value={e.exam}
                          onChange={(v) => updateEntry(e.studentId, "exam", v)}
                        />
                      </td>
                      <td className="p-3 text-center font-semibold text-base">
                        {e.final ? (
                          <span className={cn(e.final < 75 && "text-destructive")}>
                            {e.final}
                            {e.final < 75 && (
                              <TrendingDown className="size-3 inline ml-1 text-destructive" />
                            )}
                          </span>
                        ) : (
                          "—"
          ...
          <div className="flex justify-between items-center p-4 border-t border-border bg-muted/20">
            <div className="text-xs text-muted-foreground">
              🟡 Cells in red indicate potential data issues
            </div>
            <Button onClick={() => setStep(2)} className="gap-2">
              Configure Weights <ChevronRight className="size-4" />
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6 space-y-6">
            ...
            <p className="text-sm text-muted-foreground">
              Configure the weight percentages per component as prescribed by DepEd Order 8, s.2015.
            </p>
            ...
            <div className="space-y-4 max-w-md">
              <WeightSlider
                ...
                value={weights.ww}
                onChange={(v) => setWeights((w) => ({ ...w, ww: v }))}
                max={100}
              />
              <WeightSlider
                label="Performance Tasks (PT)"
                value={weights.pt}
                onChange={(v) => setWeights((w) => ({ ...w, pt: v }))}
                max={100}
              />
              <WeightSlider
                label="Quarterly Exam (QE)"
                value={weights.exam}
                onChange={(v) => setWeights((w) => ({ ...w, exam: v }))}
                max={100}
              />
            ...
            <div className="flex justify-between pt-2">
              ...
              <Button
                onClick={() => setStep(3)}
                className="gap-2"
                disabled={weights.ww + weights.pt + weights.exam !== 100}
              >
          ...
          <div className="rounded-xl border border-primary/20 bg-primary-soft/40 p-5">
            ...
            <ul className="mt-3 space-y-2 text-xs text-muted-foreground">
              <li>
                <button
                  onClick={() => setWeights({ ww: 30, pt: 50, exam: 20 })}
              ...
              <li>
                <button
                  onClick={() => setWeights({ ww: 40, pt: 40, exam: 20 })}
                  className="text-primary hover:underline"
                >
                  AP, ESP, TLE
                </button>
              ...
              <li>
                <button
                  onClick={() => setWeights({ ww: 20, pt: 60, exam: 20 })}
                  className="text-primary hover:underline"
                >
                  MAPEH
                </button>
      ...
      {step === 3 && (
        <div className="rounded-xl border border-border bg-card p-6 space-y-5">
          ...
          <p className="text-sm text-muted-foreground">
            The Grade Computation Agent will apply the configured weights, detect anomalies, and
          ...
          <div className="rounded-lg bg-muted/40 p-4 text-sm space-y-1">
            ...
            <div className="text-muted-foreground">
              Grade 6 · Math · Sampaguita · Q1 · {entries.length} students
            </div>
            <div className="text-muted-foreground">
              Weights: WW {weights.ww}% · PT {weights.pt}% · Exam {weights.exam}%
            </div>
          </div>
          ...
          <Button onClick={runComputation} disabled={computing} className="gap-2" size="lg">
            {computing ? (
              <>
                ...
                Computing {entries.length} grades…
              </>
            ) : (
              <>
            ...
            <div>
              ...
              <ul className="mt-3 text-xs text-muted-foreground space-y-1">
                <li className="flex items-center gap-2">
                  <Check className="size-3 text-success" /> Validating score ranges…
                </li>
                <li className="flex items-center gap-2">
                  <Sparkles className="size-3 text-primary animate-pulse" /> Applying weights…
                </li>
          ...
          <div className="flex justify-between pt-2">
            ...
            <Button onClick={() => setStep(4)} disabled={!computed} className="gap-2">
        ...
        <div className="space-y-6">
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="mt-2 text-3xl font-display font-semibold text-success">
                {Math.round(
                  (entries.filter((e) => (e.final || 0) >= 75).length / entries.length) * 100,
                )}
                %
              </div>
            </div>
          ...
          <div className="flex justify-between">
            ...
            <Button onClick={() => setStep(5)} className="gap-2">
              Continue to Approval <ChevronRight className="size-4" />
            </Button>
          </div>
        </div>
      )}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-xl border border-border bg-card p-6 space-y-6">
            ...
            <div className="rounded-lg bg-muted/40 p-4 space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Students computed</span>
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground">At-risk flagged</span>
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">Head Teacher (assigned)</span>
              </div>
            </div>
          ...
          <div className="rounded-xl border border-primary/20 bg-primary-soft/40 p-5">
            ...
            <ul className="mt-3 space-y-2 text-xs text-muted-foreground">
              <li className="flex items-center gap-2">
                <Check className="size-3 text-success" /> DO 8 s.2015 weights applied
              </li>
              <li className="flex items-center gap-2">
                <Check className="size-3 text-success" /> All 40 students computed
              </li>
              <li className="flex items-center gap-2 text-warning-foreground">
                <AlertCircle className="size-3" /> {anomalies.length} anomalies overridden
              </li>
            </ul>
          </div>
        </div>
